...
the time-varying gesture sequence.
The state evolves over time according to
a non-linear state transition function,
and generates an observation on each timestep
according to a non-linear observation function
that depends on both system state
and a template gesture.
The system state essentially tracks
relative time along the template gesture
and the speed at which time is moving.
A state estimate is determined
on each timestep by an inference procedure
based on particle filtering,
.... ??? more

GVF has been used successfully...

??? TODO: use, implement, experiment with

...
summarized in Figure~\ref{fig:perception}.\footnote{
  ...
  See Section~\ref{typography} and
  Figures~\ref{fig:vowels} and \ref{fig:consonants}
  for more details.}
...
Auditory cortical areas project
...
with that words
??? cite 4--6 in patterson2007.
??? patterson2007 studied
...
may have some similiarities
with the semantic hubs in
the anterior temporal lobes,
but in general
is just a convenient theoretical construct.
...
\item The upper and lower \textbf{teeth} are at the front of the mouth.
  They primarily serve as an obstacle (???) in fricative production.
...
sets of characteristics in the hierarchical levell
...
%% Or actions; note that some call them actions.
%% Also, cite goldstein2006
...
% ??? this is where 'phonemes' used to be;
% make sure this still flows
...
% Note: Bernd worries about using this for speech.
% Perhaps add in a sentence or two justifying.

%% a severe difference between modeling speech movements
%% (i.e. gestures) and arm movements First: arm movements are
%% sequential (like phonemes) but speech gestures overlap in time for
%% different articulators. May be we can argue, that for each
%% articulator (each articulatory tier), there is a phonemic sequence
%% of gestures, but these gestures need to be timed with respect to
%% other gestures occurring on other articulatory tiers . And this
%% "between tiers" temporal coordination (within phoneme coordination)
%% may be more important than the temporal seqencing of phonemes.
%% Second: gestures are simple point attractor movemetns towards ONE
%% SINGLE target while the arm movement for writing a digit comprizes
%% a sequence of around 10 target points in order to draw a
%% digit. thus, the definition of "target" is totally different for
%% digit drawing and for a gesture in speech articulation.  Third:
%% Intrinsic timing and the notion of brain oscillators may be very
%% important for speech, because there is an intrinsic timining in
%% speech (which may be not thus important for digit drawing).

...
\begin{equation}
  ...
  \delta_k(t) = \frac{\sum_{n=1}^N y_k(t+n) - y_k(t-n)}{
    2 \sum_{n=1}^N n},
    % 2 \sum_{n=1}^N n^2},  % wrong I think...
\end{equation}
...
??? conceptual figure of when switching
should happen
...
\probbox{Speech production information is decoded
  ...
  as having been produced by a static syllable representation?}
...
% a discrete iDMP is associated with
% each syllable in the vocabulary.
% While using the discrete formulation
% of the iDMP breaks the symmetry
% between the syllable recognition
% and syllable production models,
...
Finally, a \textit{temporal classification}
network is used to monitor the
state of the associative memory
and route control signals to
the rest of the model
to ensure robust performance
(it is analogous to the sequencing
and production model's
sequencer network).
...
In the recognition network, however,
...
it provides a point for
sensorimotor information
(provided by the iDMPs)
to integrate with top-down
linguistic information.
Since neuron naturally sum
input from multiple sources,
the ensemble representing syllable similarity
...
While we did not go into detail on
the mutual inhibition present in the associative memory
in Section~???,
it is important for the operation
of the syllable recognition network,
so we provide that detail here.
Figure~??? shows all of the
ensembles and connections present
??? for each input/output semantic pointer pair.
There are, in fact, two ensembles created
for each semantic pointer association
in order to suppress
??? figure out what bias is for
??? explain winner-take-all stuff

\subsubsection{Temporal classification network}

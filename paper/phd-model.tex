...
\textit{temporal transformation},
and \textit{decorrelation}.
...
??? Signal processing (LTI systems theory)
has much to teach us here;
important research activities
would include implementing these algorithms
in biologically plausible systems
to see which algorithms
can be implemented naturally
and how they perform in the context
of Sermo.

One aspect of auditory feature extraction
that warrants significant further research
is the extent to which
information from the recent past
is used at the current moment.
Until recently,
the most successful ASR systems
were based on Hidden Markov Models (HMMs),
which inherently assume that ...

??? DNNs are the best now,
but their treatment of time
is difficult to interrogate.
Many bidirectionally connected layers
of LSTMs and GRUs
appear to be sufficient for
linguistic and sensorimotor information retrieval,
but it is not clear what temporal transformations
